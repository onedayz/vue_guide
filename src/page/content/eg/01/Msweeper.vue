...
export default {
    ...
    methods: {
        ...
        checkWin() {
            ...
            if (dead || (flag !== mineTotal) || ((flag + open) !== length)) {
                return;
            }
            this.win();
        },
        ...
        openPropagation(cell) {
            ...
            const queue = [cell];
            while (queue.length) {
                queue.push(...(this.open(queue.shift()) || []));
...
.minesweeper {
    ...
    border: outset 2px $ur__color--gray-20;
    padding: 6px;
}

.board {
    border: inset 2px $ur__color--gray-20;
}
...
.indicator .smiley.ooh {
    background-position-x: -52px;
}

.indicator .smiley.dead {
    background-position-x: -78px;
}

.indicator .smiley.win {
    background-position-x: -104px;
}

.indicator .smiley:active {
